...
def deep_dive(cpe: str, port: int, user: str, password: Optional[str], 
              key_file: Optional[str], output: Optional[str], verbose: bool):
    ...
    print(f"Target CPE: {cpe}:{port} ({user})")
    print("")
    ...
    cpe_config = CpeCollectorConfig(host=cpe, port=port, username=user, password=password, private_key=key_file)
